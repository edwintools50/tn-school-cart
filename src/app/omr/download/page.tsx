...
export const metadata = {
  title: "Download the Android App — Axion Omni OMR Suite",
};

const APK_URL = "https://izaqlpeuoatetynz.public.blob.vercel-storage.com/releases/axion-omr-suite-latest.apk";

// Every step below is a stock Android/MIUI system dialog, not app-specific
// UI, so the exact same screenshots used on the main TN School Cart download
// page apply here unchanged.
const steps: { title: string; body: string; image: string }[] = [
  ...
  },
  {
    ...
    body: 'You\'ll see the real Axion Omni OMR Suite app icon and name. Tap "Install".',
  ...
  {
    title: "Done",
    body: 'You\'ll see a confirmation that the app passed all security checks. Tap "Open" to launch the app.',
    image: "/install-guide/step7-installed.png",
  },
];

export default function OmrDownloadPage() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14 w-full">
      <div className="flex flex-col items-center text-center gap-4">
        <div className="w-20 h-20 rounded-2xl overflow-hidden border border-border shadow-sm flex items-center justify-center bg-surface">
          <Image src="/omr/axion-academi-logo.png" alt="Axion Omni OMR Suite" width={80} height={80} />
        </div>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">Download for Android</h1>
          <p className="text-sm text-foreground/60 mt-1 max-w-md">
            Axion Omni OMR Suite isn&apos;t on the Play Store yet, so you&apos;ll install it directly — completely
            safe, and takes under a minute. You still need a purchased license (or an existing account) to use the
            suite once it&apos;s installed.
          </p>
        </div>
      ...
      <div className="mt-12">
        ...
        <p className="text-sm text-foreground/60 mb-6">
          These are the real screens you&apos;ll see, in order. Every Android phone shows some version of steps 2
          &amp; 3 for any app installed outside the Play Store — it isn&apos;t a warning specific to this app.
        </p>
          ...
          {steps.map((step, i) => (
            <div key={step.title} className="card p-4 rounded-2xl flex gap-3">
              <div className="w-16 shrink-0 rounded-lg overflow-hidden border border-border">
                <Image src={step.image} alt={step.title} width={1080} height={2400} className="w-full h-auto" />
              </div>
              <div>
                <p className="text-xs font-semibold text-accent uppercase tracking-wide">Step {i + 1}</p>
                <p className="font-semibold text-sm mt-0.5">{step.title}</p>
                <p className="text-xs text-foreground/60 mt-1 leading-relaxed">{step.body}</p>
              </div>
            </div>
          ))}
        </div>
